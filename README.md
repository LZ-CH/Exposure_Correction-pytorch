...
此项目为关于图像光照修正领域的论文 [Learning Multi-Scale Photo Exposure Correction.](https://arxiv.org/pdf/2003.11596.pdf)的非官方pytorch复现代码；该论文[官方代码](https://github.com/mahmoudnafifi/Exposure_Correction)为MATLAB代码；在不采用原MATLAB代码中使用的 Bilateral Guided Upsampling (bgu)上采样方式而直接采用简单的上采样方式对预测结果进行处理，复现结果为psnr: 19.756，ssim: 0.749；采用后，复现结果为psnr: 20.313 SSIM: 0.863；(原论文在相同方式下:psnr: 20.205，ssim: 0.769)
